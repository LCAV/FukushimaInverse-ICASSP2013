function [xhat, err, lambdastar, epsilonstar] = stohl_et_al_ap(A, bn, s, lambdas, epsilons, Asol)
% Solves the inverse problem using Tikhonov  and 2nd derivative smoothing regularization
%     min ||A*x - bn||^2 + lambda*||x||^2 + epsilon*||D2*x||^2
...
%   epsilons : values of the optimization parameter
%   Asol    : a-priori solution
%
...
for i=1:length(lambdas)
  for j=1:length(epsilons) % loop through all the epsilons
    %Analytic solution
    x=(A'*A+lambdas(i)*eye(n)+epsilons(j)*(D2'*D2))\(A'*bn+lambdas(i)*Asol+epsilons(j)*(D2'*D2)*Asol);
